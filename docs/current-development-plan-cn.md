...
8. **Phase 8：业务服务层与可维护性整理**
   ...
   - Bot detail API 已新增 `storageReadiness`，当 `sqlite` 被选择但 adapter 尚未实现时会明确返回 `provider_not_available`，避免 operator 误以为已经切到 SQLite。
...
9. **Phase 9：开箱即用和使用体验**
   ...
   - Operations Metrics 已新增 Conversion Funnel，显示 users seen、group trial、paid/private、successful runs，让 operator 一眼看出增长漏斗卡在哪一步。
   - Operations Metrics 已新增 Conversation Privacy Summary，明确 Operations 只显示脱敏预览、原文仍在本地 JSONL 保留，并显示最早/最新日志时间。
...
- conversation log cutoff cleanup API：dry-run / delete olderThan / admin audit
- conversion funnel：users seen / group trial / paid-private / successful runs
- conversation privacy summary：API 脱敏预览、原文 local JSONL 保留、最早/最新日志时间
...
- 时间窗口过滤
- 渠道维度对比
- conversion funnel 的时间窗口、渠道维度和历史趋势
- conversation log 自动定时清理/归档策略
...
72. 增加 Feishu Document Handling 配置骨架：operator 可选择文档处理是否启用、默认输出飞书文档/附件/两者都要、是否允许附件输入和云文档链接
73. 改进飞书非文本消息提示：明确当前先支持文本请求/飞书文档链接，附件下载上传和生成飞书文档交付需要 operator 开启并完成后续接入
...
- 飞书接入体验已经有 Quick Settings、Setup Summary、接入检查清单、Test Audience 字段和 Document Handling 输出策略；真实事件可见性、进群欢迎、真实文件下载/上传、飞书文档创建/导出还需要继续补。
...
3. 实现飞书文档处理真实链路：消息附件下载到 workspace、飞书云文档链接读取、workspace 结果上传为附件、按配置创建飞书云文档链接。
4. 把 Feishu 真实事件触发、真实可见性检查继续表单化，并把检查结果继续接入 Setup Summary。
5. 增加 Feishu 真实进群欢迎触发和更明确的付费私聊转化入口。
6. 继续完善 Quick Test 自动诊断：把 Telegram / 飞书 / 权限 / runtime 的检查结果绑定到具体表单字段，并继续补一键修复动作。
7. 数据库迁移、支付订单、worker queue、多实例并发继续作为后续工程化事项。
...
- Feishu 文档处理真实链路：附件下载、云文档链接读取、结果附件上传、飞书文档创建/导出。
...
- `storageReadiness` 已能区分 `ready`、`migration_needed`、`provider_not_available`。
